	...
	if( GetTime( ) - m_LastPingTime >= 5 )
	{
		...
		if( !m_CountDownStarted )
		{
			...
			else
			{
				...
				m_GHost->m_UDPSocket->Broadcast( 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "BN紅茶", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, slotsOpen, m_HostPort, FixedHostCounter, m_EntryKey ) );
			}
		}
